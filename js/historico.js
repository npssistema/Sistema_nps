...
let idParaEditar = null; // Controle do ID de edição
...
if (!paginacaoDiv) {
    ...
    if (tabelaResponsiva) tabelaResponsiva.insertAdjacentElement("afterend", paginacaoDiv);
}
...
function obterBadgeClassificacaoPorItem(item) {
    const classificacao = obterClassificacaoPorItem(item);
    if (classificacao === "recusa") return '<span class="classificacao-badge badge-recusa">Recusou</span>';
    if (classificacao === "promotor") return '<span class="classificacao-badge badge-promotor">Promotor</span>';
    if (classificacao === "neutro") return '<span class="classificacao-badge badge-neutro">Neutro</span>';
    if (classificacao === "detrator") return '<span class="classificacao-badge badge-detrator">Detrator</span>';
    return "";
}
        ...
        <tr class="${classeLinha}">
            ...
            <td style="min-width: 110px;">
                <div style="display: flex; gap: 5px; flex-direction: column;">
                    <button type="button" class="btn-editar" onclick="abrirModalEdicao(${a.id})">✏️ Editar</button>
                    <button type="button" class="btn-excluir" onclick="abrirModalExclusao(${a.id})">🗑 Excluir</button>
                </div>
            </td>
        </tr>
    `;
...
function aplicarFiltrosTexto(avaliacoes, filtros) {
    return avaliacoes.filter((a) => {
        ...
        if (filtros.classificacao && classificacaoDoItem !== filtros.classificacao) return false;
...
function renderizarTabelaPaginada() {
    if (!dadosFiltradosGlobais || dadosFiltradosGlobais.length === 0) {
        tbody.innerHTML = `<tr><td colspan="15">Nenhuma avaliação encontrada.</td></tr>`;
...
function renderizarPaginacao() {
    ...
    if (totalPaginas <= 1) {
        paginacaoDiv.innerHTML = `<div style="font-size:13px; color:#555;">${totalItens} avaliações encontradas</div>`;
        return;
    }
    let html = `<button onclick="irParaPagina(${paginaAtual - 1})" ${paginaAtual === 1 ? "disabled" : ""} style="margin:4px; padding:8px 12px; border:none; border-radius:6px; cursor:pointer;">◀ Anterior</button>`;
    for (let i = 1; i <= totalPaginas; i++) {
        html += `<button onclick="irParaPagina(${i})" style="margin:4px; padding:8px 12px; border:none; border-radius:6px; cursor:pointer; background:${i === paginaAtual ? "#0a3659" : "#ddd"}; color:${i === paginaAtual ? "white" : "#333"}; font-weight:${i === paginaAtual ? "bold" : "normal"};">${i}</button>`;
    }
    html += `<button onclick="irParaPagina(${paginaAtual + 1})" ${paginaAtual === totalPaginas ? "disabled" : ""} style="margin:4px; padding:8px 12px; border:none; border-radius:6px; cursor:pointer;">Próximo ▶</button>`;
    html += `<div style="margin-top:10px; font-size:13px; color:#555;">Página ${paginaAtual} de ${totalPaginas} | ${totalItens} avaliações encontradas</div>`;
...
function irParaPagina(pagina) {
    ...
    const tabelaResponsiva = document.querySelector(".table-responsive");
    if (tabelaResponsiva) window.scrollTo({ top: tabelaResponsiva.offsetTop - 20, behavior: "smooth" });
}

async function carregarHistorico() {
    esconderStatus();
    tbody.innerHTML = `<tr><td colspan="15">Carregando avaliações...</td></tr>`;
    if (paginacaoDiv) paginacaoDiv.innerHTML = "";

    const filtros = lerFiltrosDaTela();
    let query = supabaseClient.from("avaliacoes").select("*").order("id", { ascending: false });

    if (filtros.os) query = query.ilike("os_numero", `%${filtros.os}%`);
    ...
    if (error) {
        ...
        tbody.innerHTML = `<tr><td colspan="15">Erro ao carregar dados.</td></tr>`;
...
// LOGICA DE EXCLUSÃO
function abrirModalExclusao(id) { idParaExcluir = id; modalExcluir.style.display = "flex"; }
function fecharModalExclusao() { idParaExcluir = null; modalExcluir.style.display = "none"; }
async function excluirAvaliacao() {
    if (!idParaExcluir) return;
    const { error } = await supabaseClient.from("avaliacoes").delete().eq("id", idParaExcluir);
    if (error) { console.error("Erro ao excluir:", error); mostrarStatus("Erro: " + error.message, "erro"); fecharModalExclusao(); return; }
    fecharModalExclusao();
...
function abrirModalEdicao(id) {
    idParaEditar = id;
    const item = dadosFiltradosGlobais.find(x => x.id === id);
    if(!item) return;

    document.getElementById("edit_os").value = item.os_numero || "";
    document.getElementById("edit_equipamento").value = item.equipamento || "";
    ...
    document.getElementById("edit_orgao").value = item.orgao || "";
    document.getElementById("edit_comentario").value = item.comentario || "";

    modalEditar.style.display = "flex";
}

function fecharModalEdicao() { idParaEditar = null; modalEditar.style.display = "none"; }
...
async function salvarEdicao() {
    ...
    const dadosAtualizados = {
        os_numero: document.getElementById("edit_os").value.trim(),
        equipamento: document.getElementById("edit_equipamento").value.trim(),
        marca: document.getElementById("edit_marca").value.trim(),
        modelo: document.getElementById("edit_modelo").value.trim(),
        localizacao: document.getElementById("edit_localizacao").value.trim(),
        orgao: document.getElementById("edit_orgao").value.trim(),
        comentario: document.getElementById("edit_comentario").value.trim(),
    };

    const { error } = await supabaseClient.from("avaliacoes").update(dadosAtualizados).eq("id", idParaEditar);

    btnSalvar.textContent = "Salvar Alterações";
    btnSalvar.disabled = false;

    if (error) { console.error("Erro ao atualizar:", error); mostrarStatus("Erro ao atualizar: " + error.message, "erro"); fecharModalEdicao(); return; }
    
    fecharModalEdicao();
    mostrarStatus("Avaliação atualizada com sucesso.", "info");
    await carregarHistorico();
}
...
window.aplicarFiltroRapido = async function(tipo) {
    ...
    let dataInicio = "", dataFim = formatar(hoje);
    ...
    document.getElementById("filtro_data_inicio").value = dataInicio;
    document.getElementById("filtro_data_fim").value = dataFim;
...
window.addEventListener("click", function (event) {
    if (event.target === modalExcluir) fecharModalExclusao();
